...
class Autentifikasi extends CI_Controller
{
    ...
    public function logout()
    {
        ...
        $this->session->set_flashdata('pesan', '<div 
        class="alert alert-danger alert-message" role="alert">Anda telah logout!!</div>');
        redirect('autentifikasi');
    }
   ...
   Password', 'required|trim|matches[password1]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                'nama' => htmlspecialchars($this->input->post('nama', true)), 'email' => htmlspecialchars($email), 'image' => 'default.jpg', 'password' => password_hash($this->input->post('password1'), PASSWORD_DEFAULT),
                'role_id' => 2,
                'is_active' => 0,
                'tanggal_input' => time()
            ];
